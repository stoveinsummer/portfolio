...
export default function Home() {
  ...
  return <main>
    ...
    <section className="home-section home-photos">
      ...
      <div className="home-photo-grid">{recentPhotos.map((photo) => <Link href={`/photo/${photo.id}`} key={photo.id}><img src={photoSrc(photo.imageUrl.split("/").pop()!)} alt={photo.title} /><span>{photo.title}<em>{photo.takenAt}</em></span></Link>)}</div>
    </section>
    <section className="home-section record-preview">
      ...
      <div className="record-cards">
        ...
        <Link href="/tools"><p className="eyebrow">Tools · {tools.length} planned</p><h3>필요한 것을 직접 만듭니다.</h3><p>작은 개발 도구와 계산기를 하나씩 추가할 공간입니다.</p><span>도구 목록 보기 ↗</span></Link>
      </div>
    </section>
