...
export default async function PhotoDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main><SiteHeader active="Photo" /><article className="detail photo-detail"><div className="detail-head"><p className="eyebrow">{photo.category} · {photo.takenAt}</p><h1>{photo.title}</h1><p>{photo.description}</p></div><img src={src} alt={photo.title} /><dl><div><dt>Location</dt><dd>{photo.location}</dd></div><div><dt>Date</dt><dd>{photo.takenAt}</dd></div><div><dt>Theme</dt><dd>{photo.theme}</dd></div><div><dt>Color</dt><dd>{photo.color}</dd></div><div><dt>Project</dt><dd>{photo.project}</dd></div><div><dt>Tags</dt><dd>{photo.tags.join(" · ")}</dd></div></dl><Link className="back-link" href="/photo">← 사진 목록</Link></article><SiteFooter /></main>;
}
