...
export default async function InvestDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main><SiteHeader active="Invest" /><article className="detail record-detail">
    ...
    <div className="notice">개인 투자 기록이며 투자 권유가 아닙니다. 총액·평단가·계좌정보는 공개하지 않고 종목별 비중과 수량만 기록합니다.</div>
    <InvestmentDashboard record={record} startIndex={startIndex} />
    <section className="record-allocation"><div className="invest-section-head"><div><p className="eyebrow">Holdings</p><h2>보유 현황</h2></div><p>{record.portfolio.length}개 자산의 기록 시점 구성입니다.</p></div><AllocationChart items={record.portfolio} /><HoldingsTable items={record.portfolio} /></section>
