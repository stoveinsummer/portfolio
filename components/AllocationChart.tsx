...
export function AllocationChart({ items }: { items: PortfolioItem[] }) {
  const stops = items
    .map((item, index) => {
      const start = items.slice(0, index).reduce((sum, current) => sum + current.weight, 0);
  ...
  return (
    <div className="allocation">
      <div className="donut" style={{ background: `conic-gradient(${stops})` }} aria-label={items.map((item) => `${item.symbol} ${item.weight}%`).join(", ")} />
      <ul>
